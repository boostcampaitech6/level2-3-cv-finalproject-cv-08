...
from datasets import window_segment
import datasets
import models
...
from utils.mlflow_wandb import mlflow_wandb_logging
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for iter, batch in enumerate(pyprind.prog_bar(
            ...
            ###################################
            # log_mel_segs = window_segment(log_mels[0], window_length=125, stride_length=63)
            ###################################
        ...
        if epoch % args.eval_epochs == 0:
            ...
            if log_mels.dim() == 2:
                log_mel_segs = window_segment(log_mels, window_length=62, stride_length=31)
            elif log_mels.dim() == 3:
                log_mel_segs = window_segment(log_mels[0], window_length=62, stride_length=31)
            
            signature = mlflow.models.signature.infer_signature(model_input=log_mel_segs.unsqueeze(0).cpu().numpy(), model_output=model_out[0].detach().cpu().numpy())
            
            log_dict = {"val_losses":val_losses['L1_pixel_loss'], "val_inception":val_inception[0], "val_vfs":val_vfs[0], "val_facenet_L2_dist":val_facenet_L2_dist, "val_facenet_L1_dist":val_facenet_L1_dist, "val_facenet_cos_sim":val_facenet_cos_sim, "val_ih_sim":val_ih_sim, "val_recall_at_1":val_recall_at_1, "val_recall_at_2":val_recall_at_2, "val_recall_at_5":val_recall_at_5, "val_recall_at_10":val_recall_at_10, "val_recall_at_20":val_recall_at_20, "val_recall_at_50":val_recall_at_50}
            mlflow_wandb_logging(log_dict,model,signature,log_mel_segs.unsqueeze(0).cpu().numpy(),val_samples['samples'])
...
if __name__ == '__main__':
    ...
    mlflow.start_run()
    mlflow.set_experiment("sf2f_voxceleb")
    wandb.init(
            project="Voice2Face",
            config={
                "architecture": "sf2f",
